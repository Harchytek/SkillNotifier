...
void Awake()
        ...
        public static class Patch_RaiseSkill
        {
            ...
            private static float _xpBefore;
            ...
            static void Postfix(Skills __instance, Skills.SkillType skillType, float factor = 1f)
            {
                ...
                if (skillData != null && skillData.TryGetValue(skillType, out Skills.Skill skill))
                {
                    float currentXP = skill.m_accumulator;
                    float actualGain = currentXP - _xpBefore;
                    if (actualGain < 0) actualGain = currentXP; 

                    float neededXP = (skill.m_level + 1) * 20f;
                    float progressPercent = (currentXP / neededXP) * 100f;
                    ...
                    {
                        if (_cachedTotalLevel < 0f || skill.m_level != _levelBefore)
                        {
                    ...
                    if (ShowGainPourcent.Value || ShowGainXP.Value || ShowExtendedXP.Value)
                    {
                        ...
                        if (ShowGainPourcent.Value)
                        {
                            ...
                            message += $" <color={_hexPercentage}>{b.start}{progressPercent:0.00} %{b.end}</color>";
                        }
                        ...
                        if (ShowGainXP.Value)
                        {
                            ...
                            gainXpText = $" <color={_hexXPGain}>{b.start}+{actualGain:0.0}{b.end}</color>";
                        }
