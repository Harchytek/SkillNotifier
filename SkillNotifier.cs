...
namespace SkillNotifier
{
    ...
    public class SkillNotifierPlugin : BaseUnityPlugin
    {
        ...
        public static class Patch_RaiseSkill
        {
            ...
            private static float _neededXpBefore; // Sauvegarde propre globale pour le calcul de l'actualGain au Level Up
            ...
            static void Prefix(Skills __instance, Skills.SkillType skillType)
            {
                ...
                if (skillData != null && skillData.TryGetValue(skillType, out Skills.Skill skill))
                {
                    ...
                    // On stocke la valeur exacte de l'XP requise pour le niveau actuel *avant* modification
            ...
            static void Postfix(Skills __instance, Skills.SkillType skillType, float factor = 1f)
            {
                ...
                if (skillData != null && skillData.TryGetValue(skillType, out Skills.Skill skill))
                {
                    ...
                    // Calcul de la nouvelle XP visuelle
                    float currentXP = (float)Math.Round(((progressPercent / 100f) * neededXP) * 100f) / 100f;

                    // Différence pure entre nos deux valeurs calculées 
                    float actualGain = 0f;
                    if (isLevelUp)
                    {
                        // Plus besoin de recalculer avec un potentiel bug d'index, on utilise notre variable Prefix
                    ...
                    // Sécurité pour éviter les décimales flottantes type 0.240000001
